# Presentación Clase 02

Presentación HTML navegable como diapositivas.

Archivo:

```text
index.html
```

Controles:

- Flecha derecha, espacio o Page Down: avanzar.
- Flecha izquierda, Backspace o Page Up: retroceder.
- Home: primera diapositiva.
- End: última diapositiva.
- Botón `Pantalla completa`: modo presentación.

## Publicación

Si el repositorio está publicado en GitHub, se puede servir con GitHub Pages.

Opciones habituales:

- Publicar desde la rama `main` y carpeta raíz.
- Publicar desde la rama `main` y carpeta `/docs` si se mueve allí el contenido público.

Con la estructura actual, si GitHub Pages publica la rama `main` desde raíz, la URL esperada será:

```text
https://utn-frc-iaew.github.io/iaew-2026-ecommerce-api/presentaciones/clase-02/
```
